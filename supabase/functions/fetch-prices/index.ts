...
export interface FetchPricesDeps {
  ...
  now?: () => number;
  finnhubToken?: string;
  priceableProviders?: PriceablePriceProviders;
  cryptoProviders?: CryptoPriceProviders;
}

export async function handleFetchPrices(
  ...
  const cryptoPrices = await fetchCryptoPricesFlow(needCrypto, deps.cryptoProviders);
...
if (import.meta.main) {
  Deno.serve(async (req: Request) => {
    ...
    try {
      const body = await req.json() as FetchPricesRequest;
      const supabase = createClient(
        Deno.env.get("SUPABASE_URL")!,
        Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,
      );
      return json(await handleFetchPrices(body, {
        cache: createSupabaseCache(supabase),
        finnhubToken: Deno.env.get("FINNHUB_API_KEY") ?? undefined,
      }));
    } catch (err) {
