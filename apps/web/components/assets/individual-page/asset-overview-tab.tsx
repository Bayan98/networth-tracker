...
export function AssetOverviewTab({ asset, loading, assetInfo }: Props) {
  ...
  const stockAnalysisUrl =
    asset.symbol && asset.asset_type !== 'crypto'
      ? (() => {
  ...
  const analystColor = assetInfo?.analystRating
    ? assetInfo.analystRating.includes('Buy')
      ? 'var(--pos)'
      : assetInfo.analystRating.includes('Sell')
        ? 'var(--neg)'
        : 'var(--ink-2)'
    : undefined
  ...
  const marketMetrics = [
    ...
    metric(
      ...
      assetInfo?.analystRating
        ? assetInfo.analystRating + (assetInfo.analystCount ? ` (${assetInfo.analystCount})` : '')
        : null,
      analystColor,
    ),
    metric(
      'Price Target',
      assetInfo?.priceTarget != null ? formatCurrency(assetInfo.priceTarget, asset.currency) : null,
    ),
    providerLinks.length > 0 ? metric('Source', <SourceLinks links={providerLinks} />) : null,
  ...
  const companyMetrics = [
    ...
    metric(
      'Market Cap',
      assetInfo?.marketCap != null ? formatCurrency(assetInfo.marketCap, asset.currency, true) : null,
    ),
    metric('Employees', assetInfo?.employees != null ? formatCompactNumber(assetInfo.employees) : null),
    metric(
      'About',
      <span className="overview-info-copy">{assetInfo?.description}</span>,
      undefined,
      'wide',
      ...
      <div className="overview-info-grid">
        {(loading || marketMetrics.length > 0) && (
          <OverviewGroup title="Market Data">
            {loading ? (
              <OverviewLoadingMetrics count={6} />
            ) : (
              marketMetrics.map((item) => <OverviewMetric key={item.label} {...item} />)
            )}
          </OverviewGroup>
        )}
        ...
        {(loading || companyMetrics.length > 0) && (
          <OverviewGroup title="Company Info">
            {loading ? (
              <OverviewLoadingMetrics count={3} />
            ) : (
              companyMetrics.map((item) => <OverviewMetric key={item.label} {...item} />)
            )}
          </OverviewGroup>
        )}
      </div>
...
function OverviewGroup({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div>
      <div className="empty-label" style={{ marginBottom: 18 }}>
        {title}
      </div>
...
function OverviewMetric({ label, value, color, className }: MetricItem) {
  return (
    <div className={`overview-info-item ${className ?? ''}`}>
      ...
      <div className="overview-info-value" style={color ? { color } : undefined}>
        {value}
      </div>
    </div>
  )
